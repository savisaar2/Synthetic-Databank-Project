...
def test_convert_to_megabytes():
    model = LibraryModel()
    assert model.convert_to_megabytes(0) == "0 bytes"
    assert model.convert_to_megabytes(1024) == "1.00 KB"
    assert model.convert_to_megabytes(1048576) == "1.00 MB"
    assert model.convert_to_megabytes(1073741824) == "1.00 GB"

def test_sort_datasets_alphabetically():
    model = LibraryModel()
    input_data = {
        "dataset1": {},
        "dataset3": {},
    ...
    sorted_data = model.sort_datasets_alphabetically(input_data)
    expected_result = ["dataset1", "dataset2", "dataset3"]
    assert sorted_data == expected_result

def test_load_all_metadata():
    model = LibraryModel()
    metadata = model.load_all_metadata()
    assert isinstance(metadata, dict)

def test_search_metadata():
    # Create an instance of LibraryModel
    model = LibraryModel()
    ...
    model._metadata = mock_metadata

    # Define search keywords
    keywords = "breast cancer"
    
    # Call the search_metadata method
    matching_datasets = model.search_metadata(keywords)
...
def test_get_datasets_all(library_model):
...
def test_get_datasets_specific(library_model):
    # Create an instance of LibraryModel
    model = LibraryModel()
    ...
    datasets = model.get_datasets("specific", specific_datasets)
...
def test_get_file_metadata(library_model):
    # Create an instance of LibraryModel
    model = LibraryModel()
    ...
    metadata = model.get_file_metadata(dataset_name)
...
def test_add_metadata():
    ...
    model.add_metadata(test_metadata)
    loaded_metadata = model.load_all_metadata()
